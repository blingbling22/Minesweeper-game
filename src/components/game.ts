...
export default class Game {
    ...
    */
    public data: Array<Array<{ isBoom: boolean, visible_state: visible_state, mineCount: number }>>;
    public game_state: game_state;
    

    constructor(private row: number, private col: number, private boomProbability: number) {
        this.row = row;
        this.col = col;
    ...
    public init(row: number, col: number, boomProbability: number ) {
        ...
        for (let i = 0; i < this.row; i++) {
            ...
            for (let j = 0; j < this.col; j++) {
                ...
                if (Math.random() < this.boomProbability) {
    ...
    public getVisibleState(row: number, col: number) {
        let state = this.data[row][col].visible_state;
        if (state === visible_state.flag) {
            return " flag"
        }
        if (state === visible_state.number) {
            return " number"
        }
        if (state === visible_state.blank) {
            return " blank"
        }
        if (state === visible_state.boom) {
            return " boom"
        }
        if (state === visible_state.default) {
            return " default"
        }
    ...
    public open(row: number, col: number, mouseright: boolean) {
        ...
        if (this.data[row][col].visible_state == visible_state.default) {
            ...
            else if (this.data[row][col].isBoom) {
                // boom
    ...
    public openAround(row: number, col: number) {
        ...
        if (row < 0 || row >= this.row || col < 0 || col >= this.col) {
            // console.log("超出边界");
            return;
        } else if (this.data[row][col].visible_state === visible_state.default
            || this.data[row][col].visible_state === visible_state.flag
        ) {
            let mineCount = this.data[row][col].mineCount;
            if (mineCount == 0) {
                // console.log(" ${row} ${col} is blank  ");
                this.data[row][col].visible_state = visible_state.blank;
                this.openAround(row - 1, col - 1);
                this.openAround(row - 1, col);
                this.openAround(row - 1, col + 1);
